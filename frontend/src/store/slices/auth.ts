...
import { authClient, signupUser, getAuthErrorMessage, readAuthErrorResponseMeta, type AuthErrorResponseMeta } from '@/api/auth'
...
export const createAuthSlice: StateCreator<AuthSlice> = (set) => ({
  ...
  signup: async (username: string, password: string, name?: string) => {
    ...
    if (error) {
      throw new Error(getAuthErrorMessage(error, 'login', responseMeta))
    }
    if (data?.user) {
      resetUserScopedStoreState()
      set({
